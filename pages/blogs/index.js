						...
						<div className="row">
							...
							<div className="col-lg-12">
								{BlogData.map((item, index) => (
									<div className="blog-post blog-md clearfix my-5" key={index}>
										<div className="dlab-post-media dlab-img-effect zoom-slow">
											<Link href={`/blogs/${item.Page}`}><Image src={item.Thumbnail} alt="" /></Link>
										</div>
										<div className="dlab-post-info">
											<div className="dlab-post-title ">
												<h4 className="post-title"><Link href={`/blogs/${item.Page}`}>{item.BlogTitle}</Link></h4>
											</div>
											...
											<div className="dlab-post-readmore blog-share">
												<Link href={`/blogs/${item.Page}`} title="READ MORE" rel="bookmark" className="site-button outline outline-1">READ MORE
